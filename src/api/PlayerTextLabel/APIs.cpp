...
DECLARE_API_ARGNUM(PlayerTextLabel, IsValid, 2, objectPtr player, objectPtr textlabel, BoolRef valid)
{
	bool ret = Runtime::Instance().GetOMPAPI()->PlayerTextLabel.IsValid(player, textlabel, valid);
	API_RETURN(bool ret, BoolRef valid);
}

DECLARE_API_ARGNUM(PlayerTextLabel, GetText, 2, objectPtr player, objectPtr textlabel, OutputStringViewPtr output)
{
...
DECLARE_API_ARGNUM(PlayerTextLabel, GetColor, 2, objectPtr player, objectPtr textlabel, UInt32Ref color)
{
	bool ret = Runtime::Instance().GetOMPAPI()->PlayerTextLabel.GetColor(player, textlabel, color);
	API_RETURN(bool ret, UInt32Ref color);
}

DECLARE_API_ARGNUM(PlayerTextLabel, GetPos, 2, objectPtr player, objectPtr textlabel, FloatRef x, FloatRef y, FloatRef z)
{
...
DECLARE_API_ARGNUM(PlayerTextLabel, GetAttachedData, 2, objectPtr player, objectPtr textlabel, IntRef attached_player, IntRef attached_vehicle)
{
